...
test.glm <- Data[-train,]
test.glm<-ifelse(test.glm$classe==1,"fail","pass")
train.glm<-ifelse(train.glm$classe==1,"fail","pass")
### LDA Model

library(MASS)
lda_fit <- lda(classe ~ L3_S33_F3873+
                 L0_S9_F210  +
                 L3_S30_F3774+
                 L1_S25_F2773+
                 L0_S0_F8    +
                 ...
                 L1_S25_F2435+
                 L3_S29_F3351+
                 L3_S49_F4211, data = train.glm, tol = 1.0e-15)
...
lda_roc <- roc(response = test.glm$class,
...
predict_prob <- predict(lda_fit, newdata = test.glm, type = "prob")
