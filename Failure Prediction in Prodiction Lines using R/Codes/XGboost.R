...
params<- list(objective = "binary:logistic",
              booster="gbtree",
              eta = 0.01,
              max_depth = 30, # start with 6
              min_child_weight = 2,
              subsample = 0.9,
              colsample_bytree = 0.8,  # 0.3 to 0.5
              #base_score = 0.1,
              eval_metric = "auc",
              #alpha=0,
              maximize = T)

xgtrain <- xgb.DMatrix(data = data.matrix(train.glm[,c("L3_S33_F3873",
                                                    "L0_S9_F210",
                                                    "L3_S30_F3774",
                                                    "L1_S25_F2773",
                                                    "L0_S0_F8",
                                                    ...
                                                    "L1_S25_F2435",
                                                    "L3_S29_F3351",
                                                    "L3_S49_F4211")]), label = train.glm$classe, missing = NA)
xgtest <- xgb.DMatrix(data = data.matrix(test.glm), missing = NA)

model_xgb <- xgb.train(params = params, xgtrain, nrounds = 1000, verbose = T,print_every_n = 2)

pred<-predict(model_xgb,xgtest)
View(cbind(test.glm[,"classe"],pred))
plot(pred)
summary(pred)
plot(roc(test.glm$classe, pred))

summary(pred)

pnew<-ifelse(pred>0.9554,1,0)
confusionMatrix(pnew,test.glm$classe)
