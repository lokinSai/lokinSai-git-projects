...
class InputError(Error):
    ...
    def __init__(self, expression, message):
        ...
        print("Value not set in Redis: ", expression, message)


def insert_key(key, value, db_index):
    r = redis.Redis(host="redis", db=db_index)
    if not r.set(key, value):
        raise InputError


def get_key(key, db_index):
    r = redis.Redis(host="redis", db=db_index)
    return r.get(key).decode("ascii")

def send_to_channel(channel, key, data):
    try:
        channel.basic_publish(exchange="", routing_key=key, body=data)
